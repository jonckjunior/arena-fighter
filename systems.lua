...
function Systems.movement(w, dt)
    for id in pairs(w.input) do
        ...
        local inp = w.input[id]
        local dx = (inp.rt and 1 or 0) - (inp.lt and 1 or 0)
        local dy = (inp.dn and 1 or 0) - (inp.up and 1 or 0)
        ...
        if w.position[id] then
            w.position[id].x = w.position[id].x + dx * w.speed[id].value * dt
            w.position[id].y = w.position[id].y + dy * w.speed[id].value * dt
        end

        -- flag animation
        if w.animation[id] then
            w.animation[id].isPlaying = (dx ~= 0 or dy ~= 0)
        end
        ::continue::
    end
end
